...
-- This migration corrects all existing PS rows via INSERT ... ON CONFLICT DO UPDATE
-- (idempotent upsert, PostgreSQL-compatible) and adds the four stations that were
-- genuinely absent in V6:
--   PS-12 Athi River, PS-21 Naivasha, PS-22 Nakuru booster, PS-25 Nakuru junction.

-- ── Main line: correct segment labels ────────────────────────────────────────
INSERT INTO dim_asset (asset_id, asset_type, nearest_site_code, segment, chainage_km_approx, latitude, longitude, flood_landslide_risk_zone, sensor_suite)
VALUES ('PS-01', 'pump_station', 'site-002', 'Mombasa',                      NULL, -4.051253, 39.666512, NULL,             'pressure,flow,vibration')
ON CONFLICT (asset_id) DO UPDATE SET
    nearest_site_code         = EXCLUDED.nearest_site_code,
    segment                   = EXCLUDED.segment,
    latitude                  = EXCLUDED.latitude,
    longitude                 = EXCLUDED.longitude,
    flood_landslide_risk_zone = EXCLUDED.flood_landslide_risk_zone;
