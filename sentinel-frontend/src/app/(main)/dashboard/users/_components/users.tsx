...
import { fetchUsers } from "@/lib/sentinel/api";
import type { SentinelUser } from "@/lib/sentinel/auth-types";
...
function toUserRow(u: SentinelUser): UserRow {
  ...
  // lastActive: minutes since last login (0 = online now)
  let lastActive = 90 * 24 * 60; // default: very old
  if (u.lastLoginAt) {
    try {
      const diff = Date.now() - new Date(u.lastLoginAt).getTime();
      lastActive = Math.max(0, Math.floor(diff / 60000));
    } catch {
      // keep default
    }
  }
  ...
  const teamMap: Record<string, UserRow["team"]> = {
    Admin: "Platform",
    "HSE Manager": "Compliance",
    Auditor: "Compliance",
    Analyst: "Internal Tools",
    Viewer: "Internal Tools",
  };

  const status = u.status as UserRow["status"];
  ...
  return {
    ...
    name: u.name,
    role: u.role,
    status: ["Active", "Pending invite", "Deactivated", "Locked", "Suspended"].includes(status)
      ? status
      : "Active",
...
export function Users() {
  ...
  React.useEffect(() => {
    setIsLoading(true);
    fetchUsers(token)
      .then((users) => setRows(users.map(toUserRow)))
      .catch(() => {
        // Backend not available — load mock data as fallback
        import("./data").then(({ users: mockUsers }) => setRows(mockUsers));
      })
  ...
  return (
    <>
        ...
        <CardContent className="flex flex-col gap-4 px-0">
          ...
          <div className="flex items-center justify-between gap-3 px-4">
            <div className="text-muted-foreground text-sm tabular-nums">
              {isLoading ? "Loading…" : `${selectedCount} selected`}
            </div>
      ...
      <AddUserDialog open={addOpen} onOpenChange={setAddOpen} onUserCreated={handleUserCreated} />
    </>
  );
}
